...
class KakaoMap extends Component {
    ...
    getData() {
		...
		var url = 'https://express-server10.run.goorm.io/openapi/getdata?keyword=' + this.state.keyword + '&pageNo=' +  this.state.pageNo;;
        fetch(url, { method: 'get' }) //체인방식으로 실행. 장점은 줄 순서대로 각각 실행 결과가 마무리 된 후 다음 줄이 실행 된다.
            ...
            }).catch((err) => console.log('에러: ' + err + '때문에 접속할 수 없습니다.'));// then(content) 끝; //.then함수 끝 추가.
